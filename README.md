...
Useful options: `--dry-run`, `--wait` (wait until the app shows up as installed; works for Store packages, not for apps the Store installs with a vendor's own installer), `--force`, `--no-verify`, `--dir <path>`, `--market <CC>`, `--locale <tag>`, `--json`, `--exact`, `--limit <1-20>`. Run `ghostget --help` for all of them.
...
- **No elevation, no settings changes.** `doctor` prints the commands that would fix a problem; it never runs them.
